...
The MAN population comprises only 10 individuals, which is insufficient for reliable LD estimation. Accordingly, linkage network analysis is restricted to the AUT and NEW populations, and only SNPs shared between those two populations are used for comparisons. Population-specific files are generated for all three populations to preserve the complete analytical record, but MAN data are retained for reference only and are not included in downstream comparisons.
...
MAN files for chromosomes 2 and 3 are excluded from the linkage network analysis given the small sample size (n = 10). The chromosome 1 MAN file is retained to illustrate how an insufficient sample size introduces bias in LD estimation.

...
```{r read-plink-pairwise, message=FALSE, results='hide'}
pop_pairwise <-
  read.PLINK(
    here(
...
```{r check-pairwise-fst-df}
pop_pairwise2_df
```

We can subset the object
```{r subset-fst-pvalue-columns}
...
```{r plot-fst-heatmap, fig.width=4, fig.height=4}
...
```{r read-plink-sex-pairwise, message=FALSE, results='hide'}
sex_pairwise <-
  read.PLINK(
    here(
      "output", "fst", "sex.raw"
...
Lets check if the matrix is symmetric.
```{r check-sex-fst-matrix-symmetric}
isSymmetric(aa)
```


We will also add NA on the upper left side of the matrix.
```{r mask-sex-fst-upper-tri}
aa[lower.tri(aa)] <- NA
```
...
Now lets plot the data with ggplot.
You can click in the little square on the top left of the plot to open it on a new window. It will have the right proportions.
```{r plot-sex-fst-heatmap, fig.width=6, fig.height=6}
pairfst.f <- ggplot(pairfst.long, aes(Var1, Var2)) +
  geom_tile(aes(fill = value), colour = "white") +
  scale_fill_gradient(
    low = "white",
...
Clean env and memory
```{r clean-env-memory-sliding-window}
# Remove all objects from the environment
rm(list = ls())
...
Combine the data
```{r combine-pop-sliding-window-data}
...
Now we plot
```{r plot-pop-sliding-window-fst, fig.height=7, fig.width=9}
...
Clean env and memory
```{r clean-env-memory-sex-sliding}
# Remove all objects from the environment
rm(list = ls())
...
NEW males vs AUT males
```{r read-sliding-window-new-aut-males}
# Define the file path using here
...
NEW females vs AUT females
```{r read-sliding-window-new-aut-females}
# Define the file path using here
...
Combine the data
```{r combine-sex-sliding-window-data}
# Add a new column 'comparison' to each tibble
...
Now we plot
```{r plot-sex-sliding-window-fst, fig.height=7, fig.width=9}
# Calculate the middle position
combined_data$mid_pos <- with(combined_data, (BIN_START + BIN_END) / 2)
...
```{r find-overlapping-high-fst-windows}
...
```{r filter-snps-in-high-fst-window}
...
```{r filter-genes-in-high-fst-window}
...
```{r count-snps-in-high-fst-window}
# How many SNPs
length(unique(fst_cluster_chr2$SNP))
```
Check it
```{r view-fst-cluster-chr2}
fst_cluster_chr2
```


Is this gene also DE?
```{r check-fst-genes-de}
fst_cluster_chr2_de <- fst_cluster_chr2[fst_cluster_chr2$Gene_ID %in% gene_expression$Gene_ID, ]
# no it is not DE expressed
```

Now we can check if any SNP from the LD cluster 14 is on this Fst peak. Cluster ID `3215_0.78` from LDna v2.15 corresponds to `3139_0.78` from the original v.63 analysis — same SNPs, same LD threshold (see File S4e for details).

```{r load-aut-chr2-ld-clusters}
aut_ch2 <- readRDS(here("output", "ldna", "pop", "chr2", "AUT_clusters_snps.rds"))
str(aut_ch2$`3215_0.78`)
```

```{r check-ld-cluster-fst-overlap}
fst_ld_cluster <- fst_cluster_chr2[fst_cluster_chr2$SNP %in% aut_ch2$`3215_0.78`, ]
# none of them
```

We can increase the window, for example check 100kb flanking windows
```{r filter-scaffold-242-genes}
# Filter the data for Scaffold 2 and Position range
